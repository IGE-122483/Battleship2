...
public class Move implements IMove {
	...
	public String processEnemyFire(boolean verbose) {
		...
		for (int i = 0; i < this.shotResults.size(); i++) {
			...
			IPosition shot = this.shots.get(i);

			String coordenada = shot.toString();
			String estadoJogada;

			if (!result.valid()) {
				estadoJogada = "Exterior";
			} else if (result.repeated()) {
				repeatedShots++;
				estadoJogada = "Repetido";
			} else {
				validShots++;
				...
				if (result.ship() == null) {
					...
					if (result.sunk()) {
						...
						estadoJogada = "Afundou";
					} else {
						estadoJogada = "Acertou";
					}
				}
			}

			GameTracker.registarJogada("Jogada " + this.number + " - " + coordenada + " - " + estadoJogada);
		}
		...
		if (verbose) {
			...
			if (validShots == 0 && repeatedShots > 0) {
				...
				if (validShots > 0) {
					output.append(validShots).append(" tiro").append(validShots > 1 ? "s" : "")
							.append(" válido").append(validShots > 1 ? "s" : "").append(": ");
				}
				...
				if (!hitsPerBoat.isEmpty()) {
					for (Map.Entry<String, Integer> entry : hitsPerBoat.entrySet()) {
						...
						if (!sunkBoatsCount.containsKey(boatName)) {
							output.append(hits).append(" tiro").append(hits > 1 ? "s" : "")
									.append(" num(a) ").append(boatName).append(" + ");
						}
					}
				}

				if (missedShots > 0) {
					output.append(missedShots).append(" tiro").append(missedShots > 1 ? "s" : "")
							.append(" na água");
			...
			if (outsideShots > 0) {
				...
				output.append(outsideShots).append(" tiro").append(outsideShots > 1 ? "s" : "")
						.append(" exterior").append(outsideShots > 1 ? "es" : "");
			}
